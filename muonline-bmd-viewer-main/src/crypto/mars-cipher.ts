// MARS block cipher - decryption only
// Ported from C# MARSCipher.cs (custom implementation with S-box)

export class MARSCipher {
    private static readonly BLOCK_SIZE = 16;
    private readonly lKey = new Uint32Array(40);

    constructor(key: Uint8Array) {
        this.setKey(key.slice(0, 16));
    }

    getBlockSize(): number {
        return MARSCipher.BLOCK_SIZE;
    }

    blockDecrypt(inBuf: Uint8Array, len: number, outBuf: Uint8Array): void {
        for (let i = 0; i < len; i += MARSCipher.BLOCK_SIZE) {
            this.decryptBlock(inBuf, i, outBuf, i);
        }
    }

    private setKey(inKey: Uint8Array): void {
        const T = new Uint32Array(15);

        for (let i = 0; i < inKey.length / 4; i++) {
            T[i] = readU32LE(inKey, i * 4);
        }
        T[inKey.length / 4] = (inKey.length / 4) >>> 0;
        ...
        for (let j = 0; j < 4; j++) {
            ...
            for (let k = 0; k < 4; k++) {
                for (let i = 0; i < 15; i++) {
                    T[i] = rotl((T[i] + S_BOX[T[(i + 14) % 15] % 512]) >>> 0, 9);
                }
            }

            for (let i = 0; i < 10; i++) {
                this.lKey[10 * j + i] = T[(4 * i) % 15];
            }
        }

        for (let i = 5; i < 37; i += 2) {
            let w = (this.lKey[i] | 3) >>> 0;
            const m = genMask(w);
            if (m !== 0) {
                w = (w ^ (rotl(S_BOX[265 + (this.lKey[i] & 3)], this.lKey[i - 1] & 31) & m)) >>> 0;
            }
            this.lKey[i] = w;
        }
    }

    private decryptBlock(inBlk: Uint8Array, inOff: number, outBlk: Uint8Array, outOff: number): void {
        const K = this.lKey;
        let d = (readU32LE(inBlk, inOff) + K[36]) >>> 0;
        let c = (readU32LE(inBlk, inOff + 4) + K[37]) >>> 0;
        let b = (readU32LE(inBlk, inOff + 8) + K[38]) >>> 0;
        let a = (readU32LE(inBlk, inOff + 12) + K[39]) >>> 0;

        // Forward mixing
        [a, b, c, d] = fMix(a, b, c, d); a = (a + d) >>> 0;
        [b, c, d, a] = fMix(b, c, d, a); b = (b + c) >>> 0;
        [c, d, a, b] = fMix(c, d, a, b);
        [d, a, b, c] = fMix(d, a, b, c);
        [a, b, c, d] = fMix(a, b, c, d); a = (a + d) >>> 0;
        [b, c, d, a] = fMix(b, c, d, a); b = (b + c) >>> 0;
        [c, d, a, b] = fMix(c, d, a, b);
        [d, a, b, c] = fMix(d, a, b, c);
        ...
        [a, b, c, d] = rKtr(a, b, c, d, K, 34);
        [b, c, d, a] = rKtr(b, c, d, a, K, 32);
        [c, d, a, b] = rKtr(c, d, a, b, K, 30);
        [d, a, b, c] = rKtr(d, a, b, c, K, 28);
        [a, b, c, d] = rKtr(a, b, c, d, K, 26);
        [b, c, d, a] = rKtr(b, c, d, a, K, 24);
        [c, d, a, b] = rKtr(c, d, a, b, K, 22);
        [d, a, b, c] = rKtr(d, a, b, c, K, 20);
        [a, d, c, b] = rKtr(a, d, c, b, K, 18);
        [b, a, d, c] = rKtr(b, a, d, c, K, 16);
        [c, b, a, d] = rKtr(c, b, a, d, K, 14);
        [d, c, b, a] = rKtr(d, c, b, a, K, 12);
        [a, d, c, b] = rKtr(a, d, c, b, K, 10);
        [b, a, d, c] = rKtr(b, a, d, c, K, 8);
        [c, b, a, d] = rKtr(c, b, a, d, K, 6);
        [d, c, b, a] = rKtr(d, c, b, a, K, 4);

        // Backward mixing
        [a, b, c, d] = bMix(a, b, c, d);
        [b, c, d, a] = bMix(b, c, d, a); c = (c - b) >>> 0;
        [c, d, a, b] = bMix(c, d, a, b); d = (d - a) >>> 0;
        [d, a, b, c] = bMix(d, a, b, c);
        [a, b, c, d] = bMix(a, b, c, d);
        [b, c, d, a] = bMix(b, c, d, a); c = (c - b) >>> 0;
        [c, d, a, b] = bMix(c, d, a, b); d = (d - a) >>> 0;
        [d, a, b, c] = bMix(d, a, b, c);

        writeU32LE((d - K[0]) >>> 0, outBlk, outOff);
        writeU32LE((c - K[1]) >>> 0, outBlk, outOff + 4);
        writeU32LE((b - K[2]) >>> 0, outBlk, outOff + 8);
        writeU32LE((a - K[3]) >>> 0, outBlk, outOff + 12);
    }
}
...
function genMask(x: number): number {
    let m = ((~x ^ (x >>> 1)) & 0x7FFFFFFF) >>> 0;
    m = (m & (m >>> 1) & (m >>> 2)) >>> 0;
    m = (m & (m >>> 3) & (m >>> 6)) >>> 0;
    if (m === 0) return 0;
    m = (m << 1) >>> 0;
    ...
    m = (m | (m << 2)) >>> 0;
    m = (m | (m << 4)) >>> 0;
    m = (m | ((m << 1) & (~x >>> 0) & 0x80000000)) >>> 0;
    return (m & 0xFFFFFFFC) >>> 0;
}

function fMix(a: number, b: number, c: number, d: number): [number, number, number, number] {
    let r = rotr(a, 8);
    b = (b ^ S_BOX[a & 255]) >>> 0;
    b = (b + S_BOX[(r & 255) + 256]) >>> 0;
    r = rotr(a, 16);
    a = rotr(a, 24);
    c = (c + S_BOX[r & 255]) >>> 0;
    d = (d ^ S_BOX[(a & 255) + 256]) >>> 0;
    return [a, b, c, d];
}

function bMix(a: number, b: number, c: number, d: number): [number, number, number, number] {
    let r = rotl(a, 8);
    b = (b ^ S_BOX[(a & 255) + 256]) >>> 0;
    c = (c - S_BOX[r & 255]) >>> 0;
    r = rotl(a, 16);
    a = rotl(a, 24);
    d = (d - S_BOX[(r & 255) + 256]) >>> 0;
    d = (d ^ S_BOX[a & 255]) >>> 0;
    return [a, b, c, d];
}

function rKtr(a: number, b: number, c: number, d: number, K: Uint32Array, i: number): [number, number, number, number] {
    let r = Math.imul(a, K[i + 1]) >>> 0;
    a = rotr(a, 13);
    const m = (a + K[i]) >>> 0;
    const l0 = S_BOX[m & 511];
    r = rotl(r, 5);
    let l = (l0 ^ r) >>> 0;
    c = (c - rotl(m, r & 31)) >>> 0;
    r = rotl(r, 5);
